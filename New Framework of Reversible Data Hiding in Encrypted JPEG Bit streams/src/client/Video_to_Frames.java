...
public class Video_to_Frames extends Frame implements ControllerListener {
     ...
     */
       ...
       {
    	  Video_to_Frames vf=new Video_to_Frames("D:\\Robust video\\Splited video\\crt\\video1.mpg");
      }

    public Video_to_Frames(String path) {
    	System.out.println("Video path isss  ::  "+vid_path);
    	MediaLocator ml;
         File pp=new File(path);
         vid_path=pp.getName();
        
        String args = "file:/"+path;
        System.out.println("read from vedio path is "+path+ "  args value is :");
        JavaDecoder jd = new JavaDecoder();       
        ...
        if (!open(ml)) {
            ...
            File f=new File("D:/Robust video/Frames/ff.jpg");
            Show_Frames sf=new Show_Frames(f,1);
            sf.setVisible(true); 
        }
    }
    ...
    private void imageProfile(VideoFormat vidFormat)
    {
        ...
        System.out.println("Video path is ^^^^^^^^^^^^^ "+vid_path);
        String a[]=vid_path.split("\\.");
        String nn=a[0].toString();
        System.out.println("name$$$$$$$$$$$$$$$4 "+nn);
         f=new File("D:/Robust video/Frames/"+nn);
        System.out.println("Frame path is  :;"+f);
        try
        {
        	if(f.mkdir())
        	{
        		System.out.println("Directory is kkkk  :; ");
        	}
        	else
        	{
    ...
    private void useFrameData(Buffer inBuffer) {
        countFr++;
        if (countFr < startFr || countFr > endFr) {
            return;
        }
        ...
        try {
            ...
            {
                if (outvid == null) {
                    outvid = new int[imgWidth * imgHeight];
                }
             ...
             //   String sizedname = paddedname.substring(paddedname.length() - 20);
                ...
                if (sunjava)
                {
                    saveJpeg(outputImage, "frame_" + countFr + ".jpg");
                ...
                else 
                {
                    if (e == null)
                    {
                        initJpeg((RGBFormat) inBuffer.getFormat());
                    }
                    byte[] b = fetchJpeg(inBuffer);
                    String filename = "frame_" + countFr + ".jpg";
                    makeFile(filename, b);
                }
            }
        } catch (Exception e) {
    ...
    public void tidyClose() {
        ...
        System.out.println("Count no issssss............"+countFr+"Frame Path issssssss **********  "+f);
       Show_Frames sf=new Show_Frames(f,countFr);
        sf.setVisible(true); 
    ...
    private void setImage(int[] outpix) {
        if (sourceImage == null) {
            sourceImage = new MemoryImageSource(imgWidth, imgHeight, dcm, outpix, 0, imgWidth);
        }
    ...
    public void saveJpeg(Image img, String filename) 
    {
        ...
        try {
            ...
            fw = new BufferedOutputStream(new FileOutputStream(f+sep+filename));
        } catch (IOException e) {
            System.out.println("makeFile " + e);
        }
    ...
    public void makeFile(String filename, byte[] b) {
        ...
        String ext=a[1].toString();
        System.out.println("File Name is   :;"+name+"Extension is ::"+ext);
        boolean s=new File("D/images"+name).mkdir();
        if(s)
        {
